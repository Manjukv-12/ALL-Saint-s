...
const SectionTitle = ({
  ...
  <div className={`${centered ? 'text-center' : ''} ${className}`}>
    ...
    <h2 className={`text-h2 mb-4 ${light ? 'text-primary-foreground' : 'text-foreground'}`}>
      {title}
    </h2>
    {subtitle != null && subtitle !== '' && (
      <p className={`text-body max-w-2xl ${centered ? 'mx-auto' : ''} ${light ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
        {subtitle}
      </p>
    )}
  </div>
